...
class BcnnModel(object):
    """Simple MNIST Bilinear CNN Model -- ONE CNN Network, Two CNN Data Stream!!!!!!"""
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('feature_extractor1'):
            ...
            conv1 = tf.transpose(h_pool1, perm=[0, 3, 1, 2])
            conv1 = tf.reshape(conv1, [-1, 48, 14 * 14])
            conv1_T = tf.transpose(conv1, perm=[0, 2, 1])

            phi_I = tf.matmul(conv1, conv1_T)

            phi_I = tf.reshape(phi_I, [-1, 48 * 48])

            phi_I = tf.divide(phi_I, 196.0)

            y_sqrt = tf.multiply(tf.sign(phi_I), tf.sqrt(tf.abs(phi_I) + 1e-12))

            self.z_l2 = tf.nn.l2_normalize(y_sqrt, dim=1)
        ...
        with tf.variable_scope('label_predictor'):
            ...
            h_fc0 = tf.nn.relu(tf.matmul(self.z_l2, W_fc0) + b_fc0)
